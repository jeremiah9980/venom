#!/usr/bin/env python3
"""Sync the Texas Venom 14U NCS tournament dashboard from PlayNCS."""
...
OUT = ROOT / "assets" / "data" / "ncs-tournament-14u.json"
TEAM_URL = "https://www.playncs.com/fastpitch/Teams/Details/87549/texas-venom-14u"
SCHEDULE_URL = ""  # Set to the 14U event schedule URL once published
...
def parse_upcoming(html: str) -> list[dict]:
    ...
    for anchor in soup.find_all("a", href=EVENT_ID_RE):
        ...
        divisions_match = re.search(r"((?:\d{1,2}U(?:\s*[\xb7,/&]\s*)?)+)", text, re.I)
        seen.add(event_id)
        events.append({
            "id": event_id,
            "name": name,
...
def main() -> int:
    ...
    if games:
        ...
        payload["sync_message"] = "No 14U OPEN schedule has been published on NCS yet. The dashboard will update automatically once a tournament is posted."

    if upcoming:
        payload["upcoming_tournaments"] = upcoming

    payload["generated_at"] = now
    payload.setdefault("event", {"name": "", "division": "14U", "source_url": TEAM_URL})
    payload.setdefault("team", {"id": "87549", "name": TEAM_NAME, "age_group": "14U", "source_url": TEAM_URL})
    payload["source"] = {
